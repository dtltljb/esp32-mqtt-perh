...
//Instead of calculating the offsets for each pixel we grab, we pre-calculate the valueswhenever a frame 
//changes, then re-use
...
{
    if (frame!=prev_frame) {
        //We need to calculate a new set of offset coefficients. Take some random sines as offsets to 
        //make everything
